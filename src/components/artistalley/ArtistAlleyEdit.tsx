...
import { artistAlleyService, ArtistAlleyResponse } from "@/services/artistalley";
...
export const ArtistAlleyEdit = ({ prefill, mode, onDismiss }: ArtistAlleyEditProps) => {
    ...
    const doSubmit = useCallback(
        async (data: ArtistAlleySchema) => {
            setIsLoading(true);
            toast("notice", t("submit_in_progress"));
            try {
                await artistAlleyService.postTableRegistrationRequest(data);
    ...
    return (
        <FormProvider {...form}>
            ...
            <Button style={styles.button} onPress={form.handleSubmit(doSubmit)} disabled={isLoading}>
